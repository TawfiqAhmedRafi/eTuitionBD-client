...
const AllPayments = () => {
  ...
  if (isError)
    return (
      <div className="p-10 text-center text-error">Failed to load payments</div>
    );
          ...
          <tbody>
            {payments.map((p, index) => (
              ...
              >
                ...
                <td className="py-2 px-3 font-mono text-xs max-w-40 truncate">
                  {p.transactionId}
                </td>

                <td className="py-2 px-3 font-semibold text-accent">৳{p.amount}</td>
                <td className="py-2 px-3 font-semibold text-primary">
                  ৳{Number(p.amount) - Number(p.salary)}
                </td>
                ...
                <td className="py-2 px-3 text-sm">
                  <button onClick={()=>setSelected(p)} className="btn btn-primary btn-sm">View</button>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  <dialog className="modal modal-open">
    <div className="modal-box max-w-lg">
      ...
      <div className="mb-4">
        <p className="text-sm text-neutral-content">Total Paid</p>
      ...
      <div className="mt-3 text-xs text-neutral-content space-y-1">
        <p>
          Stripe ID:{" "}
