...
const TutorCard = ({ tutor }) => {
  ...
  const experience =  experienceYears || experienceMonths
    ? `${experienceYears || 0} yr${experienceYears === 1 ? "" : "s"} ${experienceMonths || 0} mo${experienceMonths === 1 ? "" : "s"}`
    : "N/A"; 
  ...
  return (
    <article className="card bg-base-100 p-5 rounded-lg shadow-sm">
      ...
      <div className="mt-4 flex items-center justify-between">
        <a className="link" href={`/tutors/${tutor._id}`}>
          View Profile
        </a>
        <a className="btn btn-sm btn-outline" href={`/message/${tutor._id}`}>
          Contact
        </a>
      </div>
    </article>
